...
export default function slideFn() {
  ...
  // 이유는? 다른 슬라이드 박스를 카피하여 만든 경우 동일한 기능이 될 수 있게 해준다.
  // (2) 이벤트대상: .abtn
  ...
  // (5) 최초 슬라이드 li에 data-seq 속성 만들고 순번넣기
  // 왜 넣는가? slide li 순서가 계속 변경되므로 블릿 인디케이터의 표시 순서를 잡기 위해 넣어준다.
  firstSlide.forEach((el, idx) => {
    // 속성세팅은 setAttribute(속성명,값)
    el.setAttribute("data-seq", idx);
  }); // forEach //

  // 슬라이드 개수 변수할당
  // 보통 변경없이 사용하는 변수는 상수라고 하고, 상수는 보통 대문자로 쓴다.
  const SLIDE_CNT = firstSlide.length;
  console.log(SLIDE_CNT);
  ...
  function goSlide() {
    ...
    if (isRight) {
      ...
      // 슬라이드 이동 후 실행해야 함
      // 따라서 setTimeout으로 시간 지연 실행 코드 작성
      setTimeout(() => {
        // [2] 맨앞 li를 선택하여 맨 뒤로 이동하기
        slide.appendChild(list[0]);
        // 슬라이드.appendChild(맨앞 li)

        // [3] 이 때, translate값 0으로 초기화 함
    ...
    // 명령어 코드는 콜스택(call stack)에서 순서대로 실행된다.
    // console.log('1');
    // 타임아웃 멧드는 태스크 큐(Task Queue)에 대기하다가
    // 콜 스택의 명령어가 모두 실행 후 큐에 쌓인 명령어를 스택으로 옮겨서 순서대로 실행한다.
    ...
    // console.log(el,idx);
    // (1) 슬라이드 data-seq값과 일치하는 순번li에 클래스 "on"넣기
    // 이때 오른쪽버튼은 1번, 왼쪽버튼은 0번째 li의 data-seq값을 읽어온다.
    let num = list[isRight ? 1 : 0].getAttribute("data-seq");
    console.log("num:", num, typeof num);

    // 속성값은 문자형이므로 비교시 === 형까지 비교할 경우, 반드시 숫자형으로 변환해야한다.
    ...
    // -> seqNum 값 즉, 슬라이드 순번과 인디케이터 li순번이 같으므로
    // 해당 순번의 li에 closs'on'을 넣고 나머지는 on을 제헌다.]
  } // goSlide //
} // sldieFn 함수 //
...
function goSlide(){
    ...
    // 2. 오른쪽버튼 여부 확인 (기본값 true 할당)
    let isRight = true;

    // 만약 this.classList가 undefined가 아니면 클래스 오른쪽 여부를 판단한다
    if(this.classList)
      isRight =  this.classList.contains('ab2');
    // 일반적으로 버튼을 클릭하지 않고 호출하면 window객체가 this로 잡히므로 classList객체가 없어서 undefined로 나오고, 하위 클래스인 contains()가 없으므로 에러메시지가 나온다. 따라서 이런 경우를 대비하여 기본값으로 isRight에 true를 주고, 직접 호출 시 오른쪽으로 이동하게 해준다. 만약 버튼을 클릭하게 되면 if문에서 걸러주므로 실제 버튼 클래스 존재 여부를 판단하여 오른쪽/왼쪽 이동버튼 분기가 작동된다.
