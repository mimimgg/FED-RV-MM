...
// 새로고침 시 스크롤위치값 캐싱 때문에 강제로 위로가기 설정
setTimeout(()=>{
    window.scrollTo(0, 0);
},100);

myFn.qs('#logo a').onclick = function(){
    pgNum = 0;
    [gnb, indic].forEach(v => addOn(v));
}
// setTimeout()으로 감싸면 스텍의 모든 코드 실행 후 나중에 실행됨
...
// 1. 자동스크롤 구현 시 필요한 초기설정하기
// (1-1) html에 부드러운 스크롤 설정하기
...
// (2-1) 페이지변수
let pgNum = 0;
// (2-2) 휠 상태변수 (광휠금지상태 : true는 막기/false는 허용)
let stsWheel = false;
...
function wheelFn(e){
    ...
    // 이동원리 : scrollTo(x축 위치값, y축 위치값) 사용이동
    // x축은 0, y축은 해당 순번의 .page 박스 위치값 넣음
    // pageEl[pgNum].offsetTop
    // -> 페이지요소들[순번].위에서부터위치값

    // (4-7) 페이지 번호와 일치하는 GNB와 인디케이터에 클래스 on넣기
    [gnb, indic].forEach(v=>addOn(v));
} // wheelFn //

// 추가분리함수 : 클래스 넣기 함수
function addOn(target){ // target - 대상요소
    // (4-8) 페이지 번호와 일치하는 GNB에 클래스 on넣기
    target.forEach((el2,idx2)=>{
        // 해당요소는 a이므로 부모인 li로 올라가서 클래스를 줘야 함
        if(idx2 == pgNum) // 해당순번과 같으면 on 넣기
            el2.parentElement.classList.add('on');
        else // 기타인 경우에는 on 제거하기
...
// 5. 메뉴 클릭 시 이동 추가기능 구현하기
// (5-1) 대상선정 : .gnb a
// const gnb = myFn.qsa('.gnb a');
// (5-2) 이벤트 설정
gnb.forEach((el,idx,list)=>{ 
    // el - 요소 / idx - 순번 / list - 전체 컬렉션
...
function movePage(evt,el,idx,list){
    // evt - 이벤트 전달변수 (함수와 직접 연결된 경우 자동전달됨)
    // -> 그러나 호출되는 일반 함수일 경우 전달해야함
    // el - 전달된 개별요소(this 대신 사용함)
    // idx - 요소의 순번전달
    // list - 전체 컬렉션 객체
    ...
    // 5) 이동한 페이지 전역변수 업데이트 하기
    pgNum = idx;
    // 메뉴 a요소의 순번을 전역페이지수에 넣어준다
    // 결과로 휠 이동시 순번이 일치하게 됨

    // 6) 동시에 gnb, indic 두가지를 모두 처리해야 함
    // -> addOn 함수를 호출한다
    [gnb, indic].forEach(v=>addOn(v));

    // [  아래 코드는 하나씩만 처리할 때 사용  ]

    // 6) a요소 컬렉션을 순회하며 해당순번과 같으면 클래스 'on'넣고 아니면 'on'제거
    // 전달변수 list로 전체 컬렉션이 들어왔으므로 이것을 활용한다.
    // list.forEach((el2,idx2)=>{
        // 해당요소는 a이므로 부모인 li로 올라가서 클래스를 줘야 함
        // if(idx2 == idx) // 해당순번과 같으면 on 넣기
            // el2.parentElement.classList.add('on');
        // else // 기타인 경우에는 on 제거하기
            // el2.parentElement.classList.remove('on');
    // }); // forEach // 
