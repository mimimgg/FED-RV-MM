...
// 4. 글자 데이터 변환하기
// - 기준 : 띄어쓰기 공백 ' '
// - 잘라서 배열 만들기 메서드 : split(자를 문자 기준)
const newText = myText.split(' ');

console.log('자름', newText);
...
console.log('aa배열', aa);
// 기존 배열값에 하트를 넣는다
console.log('하트넣은 aa배열', 
  aa.map(v=>'💗'+v));
console.log('하트넣은 aa배열을 문자열로 출력', 
  aa.map(v=>'💗'+v).join('ㅋㅋ'));
console.log('div태그로 aa배열을 싸고 문자열로 출력', 
...
myFn.addEvt(window, 'scroll', ()=>{
  ...
  console.log('오이오이오이오이', pos);

  // (2) 바운딩값이 기준값보다 작을 때 대상요소에 클래스 'on' 넣기
