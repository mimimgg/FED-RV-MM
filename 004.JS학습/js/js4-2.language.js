...
//////////////////////////////////////////////////
// 언어 세팅이 된 Json파일을 본 JS에서 불러온다.
// 단, 제이슨 파일을 불러오려면 import 문법을 써야하는데
// 모듈 문법을 사용해야 사용할 수 있다.
// 본 파일을 호출하는 html에서 type="module"이라는 속성값을
// script 요소에 적용시켜야 한다.
//////////////////////////////////////////////////

// import 변수 from 파일경로 with {type:'json'}
// -> 제이슨 파일을 부를땐 with{type:'json'}
// 즉, 함께 하는 것은 타입 제이슨이다.
// -> 경로는 반드시 모듈문법에서 상대경로를 사용해야 함
// 현재위치는 ./ , 한 폴더 위는 ../
import langData from './data_lang.json' with {type:'json'};

console.log("다국어 세팅객체:", langData);

// 나의 함수 불러오기 //
import myFn from './my_function.js';
// js확장자 생략은 SPA 개발환경에서만 가능하다.
...
function setLang(){
  // this는 호출한 선택박스요소
  // this.value는 선택된 option요소 value 속성값
  
  // 1. 선택값 읽어오기
  let optVal = this.value;
  
  // 0. 함수호출확인
  // console.log("확인", this, optVal);
  ...
  gnbList.forEach((el,idx)=>{
    ...
    // 메뉴값이 배열이므로 gnb리스트의 순번과 일치
    el.innerText = selData['메뉴'][idx];
  }); // forEach //

  // 3. 메인이지미 src 변경하기
  // 각 언어별 이미지명이 선택박스에 value값으로 만들어져 있으므로 이미지명을 optVal로 넣어준다.
