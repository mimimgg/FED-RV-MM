...
// 1. 배열 세팅하고 출력하기 //////////////////
// 1-1. new keyword로 배열 선언 및 할당하기
// 그런데 배열은 굳이 new 키워드로 선언 할 필요는 없다
// 배열 리터럴로 생성할 수 있다.
...
const arr1 = new Array(
  ...
  function () {
    // this는 누구? 호출한 요소자신
    // alert("김태리 멋짐");
    // 1. 호출한 요소박스에 김태리 사진 이미지를 넣고
    this.innerHTML += `<img src="https://nimage.g-enews.com/phpwas/restmb_allidxmake.php?idx=5&simg=2024010817235408666436d3abae512155028.jpg" alt="김태리이미지" 
    style="
    position: absolute; 
    width: 150px; 
    height: 200px; 
    ...
    // 위의 CSS 코드 체크
    // pointer-events: none;
    // 실제 이벤트는 부모박스인데 자식요소인 이미지가 매번 가림으로 인해 이벤트가 순간 없어졌다가 다시 걸렸다가 하므로 떨림 증상이 발생한다. 그래서 이 설정으로 본 이미지는 이벤트를 없애고 아래쪽 요소가 그대로 이벤트를 발생하게 된다. 따라다니는 기능에서 매우 중요한 설정이다.
    ...
    this.onmousemove = (e) => {
      ...
      // 이벤트 객체 하위 pageX, pageY는 최상단, 최왼쪽으로부터 마우스 커서의 위치를 X,Y축으로 단위없는 px값을 리턴한다.
      // 이 값은 사이트 전체를 이동하는 어떤 요소를 구현할 때 많이 사용한다.
      // console.log("이건X", e.pageX);
      // console.log("이건Y", e.pageY);

      // 여기서는 본 박스 안에서만 그 위치를 알면 되므로
      // -> offsetX, offsetY -> 해당 부모요소 박스로부터 위치를 리턴한다.
      console.log("이건X", e.offsetX);
      console.log("이건Y", e.offsetY);
...
// typeof(변수) / typeof 변수 -> 데이터 형식 출력 (object, string, number, boolean, undefined, symbol, function, bigint)
...
target[0].innerHTML = `
  ...
  데뷔년도 : ${arr1[3][0]}
`;
// 배열 안에 배열이 또 있으면 대괄호를 추가하여
// 해당 순번을 써준다. 변수[순번][순번]

// 김태리 기능 추가
// 함수호출 : 배열마지막번호 == 배열.개수-1
target[0].onclick =
  arr1[arr1.length - 1];
...
// 이런 배열과 같은 객체를 정적객체(static object)라고 부른다.
...
const arr2 = [
  ...
  function () {
    ...
    // 5초 후에 다시 원래 크기로 돌아감 : setTimeout()
    setTimeout(() => {
      this.style.scale = "1";
    }, 5000);
  },
];
// 배열 끝에 콤마는 원래는 쓰면 에러가 발생하지만 최신 브라우저에서는 사용 가능하다.
// 오히려 요즘은 추가 배열 데이터를 위해 마지막콤마를 쓰는 것이 일반적이다.

// function(){} 익명함수 -> 이름 없고 코드만 저장
// -> 배열 값으로 문자, 숫자, 배열, 객체, 함수 등을 사용할 수 있다.
...
// 배열변수명.length 는 배열 개수를 읽기/쓰기 모두 가능

const arr3 = [];
// const 상수로 리터럴 선언, 할당 후 배열값 변경은 자유롭다.
// 그러나 배열형을 변경할 수 없다. 즉, 재할당 불가능
// 그래서 const는 상수임 (코드 보안상, 안전상의 이유로 많이 사용한다)

console.log("arr3", arr3);

// 배열개수 미리 세팅하기
arr3.length = 8;
// 배열의 개수를 미리 세팅해도 배열을 더 추가할 수 있다.
// 의미는? 미리 배열 개수를 정하고, 이것을 지키려는 의도임
console.log("arr3", arr3);
...
// 현재 브라우저는 valueOf()를 안해도 배열값을 보여준다.
...
target[2].innerHTML = arr3.join("⭐️");
...
// 2. 객체 세팅 후 출력
// 2-1. new 키워드로 Object 객체 생성하여 세팅하기
// -> 실제 코드에서는 객체리터럴 방식을 사용한다.
// 즉, 변수 = {}
// 여기서는 new키워드 생성 방식도 된다는 것을 체험한다.
// 객체는 세팅시 반드시 중괄호{}를 사용한다.
// 중괄호 안에는 {속성명: 값, 속성명: 값, 속성명: 값...}
const SSG = new Object({
  ...
  팬레터: function(){
    ...
    // 7. 글자내용변경
    this.innerTxt = `손석구 최고 멋쟁이 승승장구 화이팅`;
  },
}); // SSG 객체 //
...
const GU = {
  ...
  msgFn: function(txt,ele){
    ...
    // 1)만약 함수를 별도로 호출하였으면  
    // 객체안의 메서드이므로 객체자신임
    // 2) 만약 이벤트 설정이 직접 할당되었으면 호출한 요소 자신이 this임
    console.log("this:",this);
...
target[4].innerHTML = `
  저는 ${GU.name}입니다. 
  몸무게는 비밀인데요. 그래도 말씀드리자면 ${GU.weight}입니다. 하하하 <br>
...
// 객쳉의 함수(메서드)를 클릭 이벤트에 연결한다.
// 이벤트 대상: target[4]
// -> 유의사항 : 함수에 값을 전달해야할 경우 소괄호를 사용해야 하는데, 바로 사용할 경우 소괄호를 한 함수는 바로 실행하므로 이벤트 설정에 실패하게 된다. 그리고 그 함수만 바로 실행된다. 따라서 이 호출함수를 익명함수로 감싸준다.
