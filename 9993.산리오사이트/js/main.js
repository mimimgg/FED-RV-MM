    ...
    (v) => `
      <li>
        <figure>
          ...
          <figcaption>
            <h3>${v.title}</h3>
            <p class="title-en">${v.content.split('^')[0]}</p>
            <p class="title-en">${v.content.split('^')[1]}</p>
...
// 5. 리쿠르트 영역 데이터 연결하여 출력태그 만들기
document.querySelector('.recruit-list').innerHTML =
`
  <ul class="com-type-list">
  ${recruitData.map(
    (v) => `
