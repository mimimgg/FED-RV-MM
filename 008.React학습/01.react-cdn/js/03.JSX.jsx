...
// (1) JSX를 사용한 예
// (1-1) 넣을 요소 만들기
const myEle1 = <h1>나는 JSX를 사용하는 중이다.</h1>;
// (1-2) 리액트 루트 선택하기 : createRoot() 메서드 사용
const root1 = ReactDOM.createRoot(target[0]);
// (1-3) 요소 출력하기 
// 생성된 루트에 render() 메서드를 붙여서 사용한다.
root1.render(myEle1);

// (2) JSX를 사용하지 않는 방법
// -> 넣을 요소를 createElement() 메서드로 생성하면 된다.
const myEle2 = React.createElement("h1", {}, "나는 이게 뭔말인지 하나도 모르겠어");
...
/***************************************************** 
    ...
    ((개념이해))
    예컨데 삼항연산자는 그 자리에 값을 출력할 수 있으므로 표현식이지만
    if문은 코드 전개구문이고 값이 출력되는 코드가 아니므로 표현식이 아니다. 
    또 어떤값을 할당하는 코드도 할당행위만 있고 값출력이 없으므로 표현식이 아니다!
    함수 자체는 표현식이 아니지만 리턴코드를 가진 함수를 호출하는 코드는 값을 찍어주기 때문에 표현식이다!

    참고: 
    표현식(Expression)과 대비되는 코드를 문장(Statement)이라고함
...
/***************************************************** 
    ...
    참고) label 태그의 for 속성도 for를 못씀
        대신 htmlFor라는 변형속성을 사용한다
*****************************************************/

// 7. 속성으로 클래스 세팅하여 사용하기
const myEle7 = (
