...
/* 
    ...
    (소괄호 생략 가능! 단, 소괄호든 태그든 return 키워드 바로 옆 같은 줄에 있어야 유효하다)
...
// 생성자함수를 만들어서 사용해보자
// 생성자함수는 대문자로 시작하는 이름의 function(함수)이다.

function MyFirstReact() {
  // 이것을 만든 목적 : 코드를 만들어서 Return(반환)한다.
  // 리턴코드는 반드시 하나의 Root로 감싸준다.
  return (
    <React.Fragment>
      ...
      <h2>
        ...
        빠르고 쉽게 UI화면의 구성한다! <br /># 가상돔은 실제DOM을 변경하기전에 메모리상에서 구성하는 가짜 DOM이다!{" "}
        <br />
...
// 리액트 html대상요소에 삽입하기
/* 
    가상돔을 셋팅하는 리액트 객체를 호출한다! (가상돔에 랜더링을 하는것이다.)
    -> ReactDOM
    render() -> 요소를 변경하는 메서드
