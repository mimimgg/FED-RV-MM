...
function Developer({ isNormal, isrc, ialt, itit }) {
  ...
  // 위의 if문에 들어가면 그 안의 return문 때문에 아래쪽의 return문은 실행되지 않는다.
  // 리턴 코드 만들기 ///////////
  return (
  <React.Fragment>
    <MakeDeveloper/>
...
// 음식리스트 배열변수
const foods = ["스파게티", "짜파게티", "냉면", "짜장면", "마라탕", "잡채", "김치찜"];
...
function MakeList({foodName, movieInfo}){
  ...
  return (
    <li>
      {
        // 음식 데이터가 들어온 경우에 출력됨
        // 만약 데이터가 안들어오면 undefined이므로 false 처리됨 / 할당되면 true
        foodName &&
        "개발자는 " + foodName + "좋아해"
      }
      {
        ...
        <div>
          ...
          <ul>
            ...
            gubun == "음식" &&
              ...
              /*배열변수.map() 메서드사용! 
              map메서드는 원래 새로운배열을 현재 자리에 출력하는 용도임 그러나 리액트는 이것을 변경하여 표현식안에서 출력시 태그JSX 형식으로 변환해 줌!
...
// 음식 배열값이 있는 경우 출력하기
ReactDOM.render(<WishList wList={foods} gubun="음식"/>, root[2]);

// 음식 배열값이 없는 경우 출력하기
ReactDOM.render(<WishList wList={[]} gubun="음식"/>, root[3]);
